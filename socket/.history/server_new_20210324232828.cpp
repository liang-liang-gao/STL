...
void process_data(int socket_fd) {
    ...
    while(1) {
        ...
        const char* client_addr = inet_ntop(AF_INET, (const void  *)&peer_addr.sin_addr, temp, 1024);
    ...
    while(1) {
        ...
        for(int i = 0; i < fd_size; i++) {
            if(wait_event[i].data.fd == server_fd) {//说明是建立连接的请求  
                ...
                int socket_fd = accept(server_fd, (struct sockaddr*)&client, (socklen_t *)&size_sock);
                if(socket_fd < 0) {
                    perror("accept error:");
                    continue;
                }
