...
public:
    ...
    bool threadpool_add(void (*function)(int), void * arg);//第一个参数是处理的任务(函数指针)，第二个参数是任务的参数
};
...
void *threadPool_thread(void *threadpool) {
    ...
    while(1) {//重复利用线程
        ...
        (task.function)(sock_fd, event_fd);
        //process_data(fd);
        //((task.function))(task.arg);
    }
}
//向池中加一个任务
bool threadPool::threadpool_add(void (*function)(int), void * arg) {
