...
int init_client(char* argv[]) {
	...
	if(connect(client_fd, (const struct sockaddr_in*)&client_addr, size_sock) < 0) {
...
void process_server(int client_fd, int socket_fd) {
    ...
    getpeername(socket_fd, (struct sockaddr *)&peer_addr, &peer_len); //获取对应server相应信息
    //保存server的ip和port
    char temp[1024];
    memset(temp, 0, sizeof(temp));
    char* server_addr = inet_ntop(AF_INET, &peer_addr.sin_addr, temp, 1024);
