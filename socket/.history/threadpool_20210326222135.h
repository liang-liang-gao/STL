...
threadPool *threadPool_create(int _thread_num, int _max_requests) {
    ...
    pool->threads = new pthread_t[pool->thread_num];//开辟线程数组
    if(pool->threads == nullptr) {
        perror("new threads error:");
        exit(1);
    }
    for(int i = 0; i < pool->thread_num; i++) {
        if(pthread_create(&(pool->threads[i]), nullptr, threadPool_thread, &pool) != 0) {
            //delete []pool->threads;
            perror("pthread_create error:");
            exit(1);
        }
    ...
    //2.初始化互斥锁和条件变量
    if(pthread_mutex_init(&(pool->workqueue_lock), nullptr) != 0
        ||pthread_cond_init(&(pool->workqueue_not_empty), nullptr) != 0
        ||pthread_cond_init(&(pool->workqueue_not_full), nullptr) != 0) {
        //delete []threads;
        perror("init the lock or cond error:");
        exit(1);
    }

    pool->max_requests = _max_requests;
    pool->cur_thread = 0;
    pool->cur_requests = 0;
    return pool;
 }
//线程回调函数，为啥必须在外面？？？
void *threadPool_thread(threadPool **threadpool) {
...
//向池中加一个任务
bool threadpool_add(threadPool* pool, int arg) {
    //threadPool* pool;
    pthread_mutex_lock(&(pool->workqueue_lock));//访问公共前先加锁
    //队列满则调用wait阻塞
	while(pool->cur_requests == pool->max_requests) {
		pthread_cond_wait(&(pool->workqueue_not_full), &(pool->workqueue_lock));
    }
    //插入队列中
    // threadPool_task* task;
    // task->function = function;
    // task->arg = arg;
    pool->workqueue.emplace_back(arg);
    pool->cur_requests++;
    //唤醒全部线程，争夺资源
    pthread_cond_broadcast(&(pool->workqueue_not_empty));
    pthread_mutex_unlock(&(pool->workqueue_lock));
    return true;
}
//     return 0;
