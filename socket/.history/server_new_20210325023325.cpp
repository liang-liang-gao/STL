...
private:
    int fd;
public:
    explicit FileRAII(int _fd = 0) : fd(_fd) {}
    void change(int temp) {
        fd = temp;
    }
    ~FileRAII() { if(fd != 0) close(fd); }
    int get() { return fd; }
};
...
int main(int argc, char* argv[]) {
    ...
    int flag;//阻塞非阻塞标志
    FileRAII _sock_fd[1000];
    int num = 0;//文件数目
    while(1) {
        //4.等待事件触发。这里有一个问题，虽然连接socket最好使用水平触发，但他没法和其他socket区分开，因此只能都使用边沿触发 XXX理解错了！！！
        int fd_size = epoll_wait(event_fd, wait_event, MAX_EVENT, -1);//水平触发时阻塞进行
        if(fd_size < 0) {
        ...
        for(int i = 0; i < fd_size; i++) {
            if(wait_event[i].data.fd == ser_fd) {//说明是建立连接的请求  
                ...
                //static FileRAII _sock_fd(socket_fd);//释放了？？？？？？？
                _sock_fd.change(socket_fd);
                int sock_fd = _sock_fd.get();
