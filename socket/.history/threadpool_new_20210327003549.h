...
//标记函数和参数的结构体
typedef struct {
    void (*function)(int);//函数指针,从左到右分别是返回类型，函数指针标志，参数
    int arg;//参数
...
public:
    ...
    //void *threadPool_thread(void *threadPool);//
public:
    threadPool(int _cur_threads, int _thread_num, int _max_requests);
    //~threadPool() { delete []threads; }
    bool threadpool_add(void (*function)(int), int arg);//第一个参数是处理的任务(函数指针)，第二个参数是任务的参数
};

threadPool::threadPool(int _cur_threads, int _thread_num, int _max_requests) {
...
//向池中加一个任务
bool threadPool::threadpool_add(void (*function)(int), int arg) {
    //threadPool* pool;
